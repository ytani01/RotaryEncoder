...
Task_Button::Task_Button(String btn_name, uint8_t pin,
  ...
  log_i("%s", this->btn->toString().c_str());
...
ButtonInfo_t *Task_ButtonWatcher::get_btn_info() {
  return &(this->_task_Button->btn->info);
...
void Task_ButtonWatcher::setup() {
  this->_task_Button = new Task_Button(this->_btn_name, this->_pin,
                                        this->_stack_size,
                                        this->_priority,
                                        this->_core);
  this->_task_Button->start();
...
void Task_ButtonWatcher::loop() {
  if ( this->_task_Button == NULL ) {
    return;
  }
  
  ButtonInfo_t btn_info;
  portBASE_TYPE ret = this->_task_Button->get(&btn_info);
  if ( ret == pdPASS ) {
      (*(this->_cb))(&btn_info);
  }
  task_delay(1);
